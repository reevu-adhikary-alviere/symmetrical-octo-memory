...
Alviere uses standard HTTP response codes to tell you whether a request worked. Errors that need more detail also carry a `validation` block in the response body with a specific numeric Alviere error code.
...
|------|---------|-----------------|
| `200` | OK | A successful read, or a write that returned an existing resource |
| `201` | Created | A successful write that created a resource |
| `204` | No Content | The request succeeded and there is nothing to return |
| `400` | Bad Request | Malformed request, usually a missing or invalid parameter. Carries an Alviere error code |
| `401` | Unauthorized | Missing, expired, or invalid access token |
| `403` | Forbidden | The token is valid but lacks permission for this action |
| `404` | Not Found | The resource does not exist, or is not visible to your program |
| `409` | Conflict | A duplicate `external_id`. The original operation already went through. See [Idempotency](/guides/getting-started/idempotency) |
| `500`-`504` | Server Error | An unexpected issue on Alviere's side |

:::scalar-callout{type="warning"}
Do not treat `409` as a failure to retry. It means the write you are retrying already succeeded. Look the operation up by its `external_id` and reconcile.
:::

## The validation block

Some responses carry a `validation` section describing the specific outcome:
...
| `error_code` | The numeric Alviere error code |
| `error_description` | What went wrong |

These fields are empty when the request has no issues.
...
Out of the full catalog, a handful account for most of the traffic in a new integration. These are worth handling explicitly before you ship.

| Code | Description | What to do |
|------|-------------|------------|
| `200100` | Duplicate external id | The original write succeeded. Reconcile, do not retry |
| `430001` | Idempotency error: transaction already exists in the system | Same as above, on the transaction path |
| `320001` | Account not found | Check the `account_uuid` and that it belongs to your program |
| `232001` | Account status is not ACTIVE | Finish KYC before transacting. See [Accounts](/guides/resources/accounts) |
| `420001` | Transaction not permitted because of insufficient funds | Surface a balance error to the user |
| `420002` | Funds above maximum allowed | The program transaction limit was exceeded |
| `450003` | Payment method is not active | Re-verify or re-link the payment method |
| `510004` | Issued card not active | Check the card status before authorizing |
| `100305` | Invalid Authorization token | Refresh the access token |
| `100306` | Authorization failed | Wrong credentials, or wrong environment |

## Full reference

All of the codes below are returned with an HTTP `400`.
...
| `100018` | Either client_id and client_secret or authorization token should be provided |
| `100019` | client_id and client_secret can't coexist with authorization token |
...
| `320018` | Action not permitted because Account status is not ACTIVE , CREATED or PENDING_USER |
...
| `320059` | Dossier Cannot be created because Account status is not CREATED or PENDING_USER |
| `320062` | Account is not of Wallet or Consumer type |
| `320063` | Wallet not found |
| `320070` | Action not allowed because account status is not CREATED or PENDING_USER |
...
| `320108` | Action not permitted because account status is MANUAL_REVIEW |
...
| `432005` | Bad Request: Cap is NULL or Negative for a PERCENT_CAPPED type |
...
| `492001` | Invalid Request : exchange_rate cannot be passed |
| `492002` | Invalid Request : exchange_rate is required |
...
| `510060` | Configuration error : carrier_id is missing |
| `510061` | Carrier_id is missing |
| `510062` | Carrier_id cannot be passed |
...
| `510107` | line_2 text not allowed in the request |
| `510108` | carrier_id not allowed in the request |
...
- [Idempotency](/guides/getting-started/idempotency). Why `409` and the duplicate-id codes exist.
- [Accounts](/guides/resources/accounts). Status and `status_reason`, which explain most `32xxxx` errors.
- [Transactions Overview](/guides/transactions/transactions-overview). Statuses behind the `43xxxx` family.
