...
Because the fee deducts from the card, the opening balance has to cover it. A $300 card with a $5 activation fee gives the cardholder $295. The balance has to cover the fees at creation, and a `400` is returned if it does not. That means a card that goes active has always settled its own fee.
